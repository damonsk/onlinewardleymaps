...
const defaultDataDir = process.env.DATA_DIR || path.resolve(process.cwd(), 'data');
...
export const createApp = ({dataDir = defaultDataDir}: CreateAppOptions = {}) => {
    const app = express();
...
export const startServer = async ({port = defaultPort, dataDir = defaultDataDir}: StartServerOptions = {}): Promise<Server> => {
    await fs.mkdir(dataDir, {recursive: true});
