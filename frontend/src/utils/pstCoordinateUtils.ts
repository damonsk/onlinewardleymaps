...
export function validatePSTCoordinates(coordinates: PSTCoordinates): boolean {
    ...
    const inRange = [maturity1, visibility1, maturity2, visibility2].every(coord => coord >= 0 && coord <= 1 && !isNaN(coord));
 ...
 */
export function validatePSTBounds(bounds: PSTBounds, constraints: ResizeConstraints = DEFAULT_RESIZE_CONSTRAINTS): boolean {
    return (
 ...
 */
export function isPointInPSTBounds(point: {x: number; y: number}, bounds: PSTBounds): boolean {
 ...
 */
export function snapToGrid(value: number, gridSize: number = 10, enabled: boolean = false): number {
