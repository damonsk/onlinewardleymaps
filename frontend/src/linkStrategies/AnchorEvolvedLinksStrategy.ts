import {Anchor, Link, LinkExtractionStrategy, LinkStrategy, MapElement} from './LinkStrategiesInterfaces';

export default class AnchorEvolvedLinksStrategy implements LinkExtractionStrategy {
    private links: Link[];
    ...
    getLinks(): LinkStrategy {
        const links = this.links.filter(
            li =>
                ...
                this.mapElements.getEvolvedElements().filter((i: any) => i.name === li.end),
        );

        return {
            name: 'anchorEvolvedLinks',
            links: links,
            startElements: this.anchors.map((anchor: Anchor) => ({...anchor}) as MapElement),
