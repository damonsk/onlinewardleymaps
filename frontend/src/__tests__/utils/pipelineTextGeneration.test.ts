...
pipeline Kettle
{
    ...
    component Pipeline Component 2 [0.60]
        ...
        it('should throw error for non-existent pipeline', () => {
            expect(() => insertPipelineComponent(mockMapText, 'NonExistent', 'foobar', 0.50))
                .toThrow('Pipeline \"NonExistent\" not found in map text');
        });

        it('should throw error for invalid inputs', () => {
            expect(() => insertPipelineComponent('', 'Kettle', 'foobar', 0.50)).toThrow();
            expect(() => insertPipelineComponent(mockMapText, '', 'foobar', 0.50)).toThrow();
            expect(() => insertPipelineComponent(mockMapText, 'Kettle', '', 0.50)).toThrow();
...
describe('Pipeline Workflow Integration', () => {
    ...
    it('should handle component insertion workflow', () => {
        ...
        // Insert a component
        const updatedText = insertPipelineComponent(initialText, 'Kettle', 'Electric Kettle', 0.40);
