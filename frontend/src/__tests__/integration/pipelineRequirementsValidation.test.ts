    ...
    describe('Requirement 2: 0.3 spacing between default components', () => {
        it('should space components 0.3 apart relative to pipeline position', () => {
            ...
            testCases.forEach(testCase => {
                ...
                console.log(
                    `X: ${testCase.x}, Spacing: ${actualSpacing.toFixed(2)}, Components: [${maturity1.toFixed(2)}, ${maturity2.toFixed(2)}]`,
                );
            });
        });
...
pipeline Kettle
{
    ...
    component Pipeline Component 2 [0.60]
        ...
        it('should match the exact format from user requirements', () => {
            ...
            const lines = result.split('\n');
            expect(lines.find(line => line.includes('component foobar'))).toMatch(/^\s+component foobar \[0\.50\] label \[-10, -10\]$/);
        });
    });
        ...
        const createTestMap = (): UnifiedWardleyMap => ({
            ...
            pipelines: [
                createPipeline({
                    ...
                    line: 2,
                    components: [
                        {id: 'pc1', name: 'Pipeline Component 1', maturity: 0.1, visibility: 0.57, line: 3},
                        {id: 'pc2', name: 'Pipeline Component 2', maturity: 0.9, visibility: 0.57, line: 4},
                    ],
                }),
            ],
            attitudes: [],
    ...
    describe('Requirement 5: Complete workflow validation', () => {
        it('should handle the complete user scenario end-to-end', () => {
            ...
            const withFoobar = insertPipelineComponent(initialPipeline, 'Kettle', 'foobar', 0.5, {x: -10, y: -10});
