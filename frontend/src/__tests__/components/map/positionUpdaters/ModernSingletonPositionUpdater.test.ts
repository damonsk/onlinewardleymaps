...
describe('ModernSingletonPositionUpdater', () => {
    ...
    beforeEach(() => {
        ...
        mutator = jest.fn();
        updater = new ModernSingletonPositionUpdater('evolution', mapText, mutator);
        successorUpdater = {
            update: jest.fn(),
        };
    });
    ...
    test('creates a new evolution entry when one does not exist', () => {
        ...
        expect(mutator).toHaveBeenCalledWith('component Foo [0.1, 0.2]\ncomponent Bar [0.3, 0.4]\nevolution [0.5, 0.6]');
    });
    ...
    test('delegates to successor when evolution entry already exists', () => {
        ...
        expect(successorUpdater.update).toHaveBeenCalledWith({param1: 0.5, param2: 0.6}, 'unused');
    ...
    test('correctly sets successor', () => {
        ...
        expect(successorUpdater.update).toHaveBeenCalledWith({param1: 0.7, param2: 0.8}, 'unused');
    });
});
