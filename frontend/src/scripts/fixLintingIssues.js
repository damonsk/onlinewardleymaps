...
const files = fs.readdirSync(strategyDir).filter(file => 
  file.endsWith('.ts') && 
...
files.forEach(file => {
  ...
  if (file === 'LinksBuilder.ts') {
    ...
    if (content.includes("import { ModernMapElements } from '../processing/ModernMapElements';")) {
      content = content.replace(
        /import { ModernMapElements } from '\.\.\/processing\/ModernMapElements';/,
      ...
      console.log(`${file} - No ModernMapElements import found, skipping`);
    }
  }
});
