...
- [x]   4. Add unified deletion system with keyboard shortcuts - [COMPLETED]
...
- [x] 4.1 Create SelectionManager service - [COMPLETED]
...
- [x] 4.2 Extend MapComponentDeleter for evolved components - [COMPLETED]
...
- [x] 4.3 Implement keyboard deletion workflow - [COMPLETED]
...
- [-] 8. Implement map canvas context menu functionality
  - Extend context menu system to detect canvas vs element clicks
  - Create map properties manager for DSL generation and parsing
...
- [ ] 8.2 Create MapPropertiesManager service
  ...
  - Implement validation for map property values and error handling
  - _Requirements: 10.3, 10.4, 11.4, 11.5, 12.4, 12.5_

- [ ] 8.3 Implement map style selection submenu
  - Create style submenu with Plain, Wardley, and Colour options
  - Add current style detection and visual indication in submenu
  - Implement style change action that generates "style [stylename]" DSL syntax
...
- [ ] 9.1 Create MapSizeDialog component
  ...
  - Create size confirmation that generates "size [width, height]" DSL syntax
...
- [ ] 9.2 Create EvolutionStagesDialog component
  ...
  - Create confirmation that generates "evolution stage1->stage2->stage3->stage4" DSL
...
- [ ] 9.3 Integrate dialogs with canvas context menu
  ...
  - Implement proper dialog cleanup and focus management after actions
  - _Requirements: 11.1, 11.2, 12.1, 12.2_

- [ ] 10. Write tests for map canvas context menu functionality
  - Create unit tests for MapPropertiesManager DSL parsing and generation
  - Write integration tests for canvas context menu workflows
  - Add E2E tests for complete map property editing scenarios
  - _Requirements: All map canvas requirements validation_

- [ ] 10.1 Write unit tests for map properties functionality
  - Test MapPropertiesManager parsing of existing style, size, and evolution from map text
  - Test DSL generation functions for all three property types
  - Test validation logic for size inputs and evolution stage names
  - Test error handling for invalid property values and map text parsing failures
  - _Requirements: 10.3, 10.4, 11.4, 11.5, 12.4, 12.5_

- [ ] 10.2 Create integration tests for canvas context menu
  - Test canvas right-click detection vs element right-click prioritization
  - Test map style submenu selection and immediate visual updates
  - Test dialog opening from canvas context menu actions
  - Test complete property update workflows from menu to map text changes
  - _Requirements: 9.1, 9.2, 9.4, 10.1, 10.2, 11.1, 12.1_

- [ ] 10.3 Write E2E tests for map property editing workflows
  - Test complete map style change workflow (canvas right-click → style submenu → selection → visual update)
  - Test complete map size setting workflow (canvas right-click → size dialog → input → confirmation → DSL update)
  - Test complete evolution stages editing workflow (canvas right-click → evolution dialog → stage editing → DSL update)
  - Test error scenarios and validation feedback in dialogs
  - _Requirements: 10.1, 10.2, 10.3, 10.4, 11.1, 11.2, 11.4, 12.1, 12.2, 12.4_
